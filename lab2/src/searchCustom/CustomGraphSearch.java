...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		 ...
		 * -You are free to change anything how you wish as long as the program runs,
		 * but some structure is given to help you.
		 * -You can Google for "javadoc <class>" if you are uncertain of what you can do
		 * with a particular Java type.
		 * 
		 * -SearchNodes are the nodes of the search tree and contains the relevant
		 * problem state, in this case x,y position (GridPos) of the agent
		 * --You can create a new search node from a state by: SearchNode childNode =
		 * new SearchNode(childState, currentNode);
		 * --You can also extract the state by .getState() method
		 * --All search structures use search nodes, but the problem object only speaks
		 * in state, so you may need to convert between them
		 * 
		 ...
		 * explored set means a set of SearchNode objects.
